...
const DEFAULT_OUTPUT_DIR_PATH = path.join(__dirname, 'my_codewars_solutions');
...
function generateFilename(index, language, fileName) {
  let extension = extensions[language] || 'txt';
  return index === 0
    ? `${fileName}.${extension}`
    : `${fileName}_${index + 1}.${extension}`;
}
...
async function main() {
  program
    ...
    .option(
      '-o, --output <path>',
      'path to the output directory where solutions will be saved',
      DEFAULT_OUTPUT_DIR_PATH
    )
    .option('-v, --verbose', 'explain what is being done')
    .option(
      '-d, --debug',
      'run the scraper in debug mode (will make browser window appear to see what is being done)'
    )
    .requiredOption('-u, --username <username>', 'your CodeWars username')
    .requiredOption(
      '-e, --email <email>',
      'your GitHub or CodeWars account email'
    )
    .requiredOption(
      '-p, --password <password>',
      'your GitHub or CodeWars account password'
    )
    .parse(process.argv);

  if (!program.codewars && !program.github) {
    console.error(
      chalk.redBright('You must provide a type of authentication!')
    );
    console.info(chalk.blueBright('Use --help to read about auth options'));
    exit(1);
  } else if (program.codewars && program.github) {
    console.error(
      chalk.redBright('You cannot provide both types of authentication!')
    );
    console.info(
      chalk.blueBright(
        'Only provide one type of authentication and run the command again'
      )
    );
    exit(1);
  }
  ...
  if (program.codewars) {
    // Log in using Codewars credentials
  ...
  log.info(
    `Navigating to ${CODEWARS_BASE_URL}/users/${program.username}/completed_solutions.`
  );

  await page.goto(
    `${CODEWARS_BASE_URL}/users/${program.username}/completed_solutions`,
    { waitUntil: 'domcontentloaded' }
  );
  ...
  const solutions = await page.evaluate(() => {
    return [...document.querySelectorAll('.list-item-solutions')].map(
      (solution) => {
        const problemId = solution
          .querySelector('.item-title a')
          .getAttribute('href')
          .match(/[a-z0-9]+$/g)[0];
        const problemName = solution
          .querySelector('.item-title a')
          .textContent.toLowerCase()
          .replace(/[^\w ]/gi, '')
          .trim()
          .replace(/ +/g, '_');
        const languages = [...solution.querySelectorAll('pre code')].map(
          (code) => code.getAttribute('data-language')
        );
        const codeSolutions = [...solution.querySelectorAll('code')].map(
          (code) => code.textContent
        );
        const kata = solution.querySelector('.item-title span').textContent;
